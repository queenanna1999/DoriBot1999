...
async def on_message(message):
    ...
    if message.content.startswith("!도와줘"):
        ...
        embed.add_field(name='!자살이란', value=' 도리봇이 자살의 정의들을 불러옵니다. 해당 문구는 랜덤으로 한개씩 출력됩니다 ', inline=False)
        embed.add_field(name='!음악', value=' [미완성]도리봇을 음성채널에 추가 및 음악 재생 (예시: !음악 https://www.youtube.com/watch?v=8G0xVM-X9Jk&list=WL&index=4 ) ', inline=False)
        embed.add_field(name='!종료', value=' [미완성]도리봇을 음성채널에서 내보내기 ', inline=False)
        embed.add_field(name='!스킵', value=' [미완성]도리봇이 음악을 스킵합니다 ', inline=False)
        embed.add_field(name='!목록', value=' [미완성]도리봇이 재생목록을 불러옵니다 ', inline=False)
    ...
    if message.content.startswith("!자기소개"):        #이 부분은 이 소스를 수정해서 새로운 자작봇을 만드실 분들에 해당하시는 분들만 수정 부탁드립니다.
        ...
        await message.channel.send(embed=embed)
        embed = discord.Embed(title=" MBTI는 ENFP 이고, ")
        await message.channel.send(embed=embed)
...
async def on_message(message):
	       ...
	       if message.author == app.user: #봇이 채팅을 쳤을 때 명령어로 인식되지 않음
	              	return
       	...
       	if message.content.startswith("!음악"): #음성채널에 봇을 추가 및 음악 재생
		              ...
		              try:
		                     	...
		                     	if url1 == None:
			                            	await message.channel.send(embed=discord.Embed(title=":no_entry_sign: url을 제대로 입력해주세요."))
			                            	return
	              	except IndexError:
			                     await message.channel.send(embed=discord.Embed(title=":no_entry_sign: url을 입력해주세요."))
			                     return
	              	...
	              	if app.is_voice_connected(server) and not playerlist[server.id].is_playing(): #봇이 음성채널에 접속해있으나 음악을 재생하지 않을 때
		                     	...
		                     	await message.channel.send(embed=discord.Embed(title=":white_check_mark: 추가 완료!"))
	              	...
	              	try:
		                     	...
		                     	await message.channel.send(embed=discord.Embed(title=":no_entry_sign: 음성채널에 접속하고 사용해주세요."))
			                     return
	              	...
	              	try:
			                     ...
			                     await message.channel.send(embed=discord.Embed(title=":no_entry_sign: 존재하지 않는 경로입니다."))
		                     	await voice_client.disconnect()
		                     	return
		              player.start()
	       ...
	       if message.content == "!종료": #음성채널에서 봇을 나가게 하기
		              ...
		              if voice_client == None: #봇이 음성채널에 접속해있지 않았을 때
		                     	await message.channel.send(embed=discord.Embed(title=":no_entry_sign: 봇이 음성채널에 없어요."))
		                     	return
		
		              await message.channel.send(embed=discord.Embed(title=":mute: 채널에서 나갑니다.")) #봇이 음성채널에 접속해있을 때
		              await voice_client.disconnect()

        	if message.content == "!스킵":
	              	id = message.server.id
		              if not playerlist[id].is_playing(): #재생 중인 음악이 없을 때
		                     	await message.channel.send(embed=discord.Embed(title=":no_entry_sign: 스킵할 음악이 없어요."))
			                     return
		              await clmessage.channel.send(embed=discord.Embed(title=":mute: 스킵했어요."))
		              playerlist[id].stop()
	
        	if message.content == "!목록":

		               if playlist == []:
			                      await message.channel.send(embed=discord.Embed(title=":no_entry_sign: 재생목록이 없습니다."))
			                      return
